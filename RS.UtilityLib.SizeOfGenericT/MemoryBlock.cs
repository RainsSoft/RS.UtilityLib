...
using RS.UtilityLib.SizeOfGenericT.BGRA;

namespace RS.UtilityLib.SizeOfGenericT
{
    /// <summary>
    /// 管理任意大小的内存块。您还可以创建引用父MemoryBlock分配的部分内存的子MemoryBlock。
    /// 如果父对象被释放，则子对象将无效
          ...
          IDeferredSerializable
    {
        ...
        //一次串行化1MB：这使我们能够串行化非常大的块，并在这样做的同时节省内存
        ...
        //此大小或更大的块使用AllocateLarge（VirtualAlloc）而不是Allocate（HeapAlloc）进行分配
        ...
        //如果parentBlock==null，那么我们分配了指针并负责解除分配
        //如果parentBlock！=null，那么parentBlock分配了它，而不是我们
        [NonSerialized]
        private void* voidStar;
        /// <summary>
        /// 如果voidStar为null，而这是false，那么我们知道它为null是因为分配失败。否则我们就大错特错了
        /// </summary>
        [NonSerialized]
        ...
        /// <summary>
        /// 设置一个标志，指示此MemoryBlock实例所指向的内存是否可以写入。
        ...
        /// <summary>
        /// 将字节从内存的一个区域复制到另一个区域。由于此函数适用于MemoryBlock实例，因此它可以进行边界检查。
        ...
        /// <summary>
        /// 创建一个新的父MemoryBlock并将我们的内容复制到其中
        ...
        /// <summary>
        /// 创建一个新的MemoryBlock实例，该实例引用另一个MemoryBlock的一部分。
        /// 其他MemoryBlock是父实例，此新实例是子实例。
        ...
        void IDeferredSerializable.FinishDeserialization(Stream input, DeferredFormatter context) {
            ...
            Threading.ThreadPool threadPool = new Threading.ThreadPool(Threading.Processor.LogicalCpuCount);
            ArrayList exceptions = new ArrayList(Threading.Processor.LogicalCpuCount);
        ...
        void IDeferredSerializable.FinishSerialization(Stream output, DeferredFormatter context) {
            ...
            Threading.ThreadPool threadPool = new Threading.ThreadPool(Threading.Processor.LogicalCpuCount);
            ArrayList exceptions = ArrayList.Synchronized(new ArrayList(Threading.Processor.LogicalCpuCount));
