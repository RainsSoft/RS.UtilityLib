    ...
    public static class SafeMemory {
        ...
        private static class SizeCache<T> {
            ...
            static SizeCache() {
                var type = typeof(T);
                if (type.IsEnum)
                    type = GetEnumUnderlyingType(type); //type.GetEnumUnderlyingType();
        ...
        public static Type GetEnumUnderlyingType(Type enumType) {
            ...
            //var fields = enumType.GetFields(System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Static);
           ...
           return enumType.GetFields(BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic)[0].FieldType;
            ...
            throw new ArgumentException("Enum does not have an underlying type.");
        }
        ...
        private sealed class LocalMemory : IDisposable {
            ...
            public T Read<T>(int offset = 0) => (T)Marshal.PtrToStructure(AddIntPtr(this.hGlobal, offset), typeof(T));

            public object Read(Type type, int offset = 0) => Marshal.PtrToStructure(AddIntPtr(this.hGlobal, offset), type);

            public void Write(byte[] data, int index = 0) => Marshal.Copy(data, index, this.hGlobal, this.size);

            public void Write<T>(T data, int offset = 0) => Marshal.StructureToPtr(data, AddIntPtr(this.hGlobal, offset), false);
