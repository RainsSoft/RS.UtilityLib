...
namespace ComponentOwl.BetterListView
{
	...
	public sealed class BetterListViewItemCollectionConverter : TypeConverter
	{
		...
		public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType) {
			BetterListViewItemCollection betterListViewItemCollection = (BetterListViewItemCollection)value;
			if (destinationType == typeof(string)) {
				return $"{betterListViewItemCollection.Count} item(s)";
			}
			if (destinationType == typeof(InstanceDescriptor)) {
				BetterListViewItem[] array = new BetterListViewItem[betterListViewItemCollection.Count];
				betterListViewItemCollection.CopyTo(array, 0);
				return new InstanceDescriptor(typeof(BetterListViewItemCollection).GetConstructor(new Type[1] { typeof(IEnumerable<BetterListViewItem>) }), new object[1] { array });
			}
