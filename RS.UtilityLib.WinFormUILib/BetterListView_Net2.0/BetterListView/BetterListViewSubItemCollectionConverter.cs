...
namespace ComponentOwl.BetterListView
{
	...
	public sealed class BetterListViewSubItemCollectionConverter : TypeConverter
	{
		...
		public override object ConvertTo(ITypeDescriptorContext context, CultureInfo culture, object value, Type destinationType) {
			BetterListViewSubItemCollection betterListViewSubItemCollection = (BetterListViewSubItemCollection)value;
			if (destinationType == typeof(string)) {
				return $"{betterListViewSubItemCollection.Count} sub-item(s)";
			}
			if (destinationType == typeof(InstanceDescriptor)) {
				BetterListViewSubItem[] array = new BetterListViewSubItem[betterListViewSubItemCollection.Count];
				betterListViewSubItemCollection.CopyTo(array, 0);
				return new InstanceDescriptor(typeof(BetterListViewSubItemCollection).GetConstructor(new Type[1] { typeof(IEnumerable<BetterListViewSubItem>) }), new object[1] { array });
			}
