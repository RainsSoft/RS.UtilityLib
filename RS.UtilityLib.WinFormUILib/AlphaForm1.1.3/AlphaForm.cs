...
///较小的值并没有真正的用处
///阻尼越大，初始斜坡越陡
///如果您希望它一开始是平的，那么调用1.0-Evaluate（1.0-t）
...
namespace AlphaForm
{
    ...
    /// <summary>
    /// AlphaFormMarker用作设计时辅助工具，用于指定背景图像中的一个或多个点，这些点将用于构建主窗体的区域。
    /// 在设计器中，它必须始终添加到AlphaFormTransformer控件（而不是主窗体）
    ...
    /// <summary>
    /// 将AlphaFormTransformer的单个实例添加到要转换为alpha通道边界窗口的窗体中(FormBorderStyle = System.Windows.Forms.FormBorderStyle.None;)。
    /// 它承载要在窗口上显示的所有控件、显示图像边框的分层窗口的图像，
    /// 以及一个或多个用于构建主窗体区域的AlphaFormMarkers。
    ...
    public class AlphaFormTransformer : Panel
    {
        /*
         如何使用
1:有两个控件：AlphaFormTransformer和一个辅助控件AlphaFormMarker。在您的项目中引用AlphaFormTransformer.dll并将它们添加到您的工具箱中。
2:从设计器中的表单开始，并将其设置FormBorderStyle为“无”。
3:将控件 (AFT) 的一个实例拖到AlphaFormTransformer窗体上并将其停靠到 Fill。AFT 的大小必须与表格的大小相同。
4:在表单的Load 事件处理程序（或OnLoad 覆盖）中，添加对TransformForm()
alphaFormTransformer1.TransformForm();
5:选择 32 位 Alpha 通道 PNG 或 TIF 图像作为 AFT 控件的背景图像。该图像需要具有由其 Alpha 通道和将容纳其他控件的内部透明区域遮盖的所需窗口框架元素。
6:选择表单，并将其宽度和高度设置为等于图像的宽度和高度。这是一个重要的前提。窗体、AFT 控件和 AFT 的背景图像都必须具有相同的尺寸。
就其本身而言，Alpha 通道的信息不足以让 AFT 计算显示要添加的控件所需的主窗体区域。但我们让这一切变得简单。将AlphaFormMarker(AFM) 控件拖到将显示控件的每个透明区域中。这些区域必须完全由不透明的边界界定。每个标记只需位于透明区域中的任何位置，并且其中心（由十字准线显示）必须落在透明像素上。定位后，将它们缩小到所需的大小，或者将它们放在后面以将其移开。需要注意的是，它们必须添加到 AFT，而不是任何其他容器控件。如果您决定稍后添加它们，请注意这一点。最后，它们在运行时被隐藏。
标记者的FillBorder属性指定将构造区域到非透明像素边界的深度。合成图像通常具有半透明边缘。因此，从标记位置构建的区域需要将一定数量的像素扩展到（并且从合成角度来看“在”下方）半透明区域，否则您将沿着这些边界看到桌面。您希望边框值足够大以覆盖半透明边缘的厚度（通常是几个像素），但又不能太大，否则可能会导致其延伸超过图像帧的另一侧。         
7:现在将您喜欢的任何其他控件添加到图像的内部透明区域。AFT 是一个容器控件（源自Panel），因此您将向其中添加控件。图像将始终在运行时遮盖控制区域，但不一定在设计时遮盖。如果您没有将控件的背景颜色设置为透明，则在设计器中它可能会覆盖图像的遮罩边框，因为它位于顶部，而在运行时它位于 Alpha 通道图像的下方。通过将控件的背景颜色设置为透明，您实际上可以伪造运行时行为。这样做可以帮助您布置事物，即使您稍后将其更改为不透明的颜色。并非所有控件都支持不透明背景色，
8:运行时换肤
UpdateSkin在运行时，您可以通过调用传递窗口框架的新位图来更改窗体的 Alpha 通道图像。但是，如果新位图的 Alpha 通道与当前位图不同，则*在*调用此方法之前必须执行以下操作：

如果图像的大小已更改，请调整主表单和 AFT 的大小以匹配（如果 AFT 已停靠，则仅调整表单）。
重新定位所有 AFM 标记，以便可以计算窗体的区域。
根据需要重新定位所有其他控件。
另一方面，如果您的新图像具有相同的大小和 alpha 但 RGB 不同，那么您只需调用此方法即可。源解决方案有一个示例项目，说明如何在运行时更改图像。        
         */
        #region Class Variables
        bool m_drag = false;
        Point m_dragStart;
