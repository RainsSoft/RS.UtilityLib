            ...
            if (areas.Count > 0 || !AreaManager.CurrentHoverArea.IsEmpty)
            {
                ...
                if (Config.ShowInfo)
                {
                    foreach (Rectangle area in areas)
                    {
                        if (area.IsValid())
                        {
                            ImageHelpers.DrawTextWithOutline(g, "选定区域后,双击左键或按Enter完成截图,\r\n单击右键取消区域,按ESC或双击右键退出。",
                                new PointF(area.X , area.Y - 50), SystemFonts.CaptionFont ,Color.Gold, Color.Green);
                            ImageHelpers.DrawTextWithOutline(g, string.Format("X:{0} Y:{1} 长:{2} 宽:{3}", area.X, area.Y, area.Width, area.Height),
                                new PointF(area.X + 5, area.Y - 25), textFont, Color.White, Color.Black);
                        }
                    }
                }
            }
