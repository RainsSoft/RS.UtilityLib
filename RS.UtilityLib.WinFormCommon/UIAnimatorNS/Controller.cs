    ...
    public class Controller {
        ...
        protected virtual void OnFramePainting(object sender, PaintEventArgs e) {
            ...
            if(time > animation.MaxTime)
                time = animation.MaxTime;
            if(time < animation.MinTime)
                time = animation.MinTime;
            CurrentTime = time;
        ...
        public bool IsCompleted {
            get {
                return (TimeStep >= 0f && CurrentTime >= animation.MaxTime) || (TimeStep <= 0f && CurrentTime <= animation.MinTime);
            }
        }
