...
namespace Silver.UI {
    ...
    public class ToolBox : UserControl {
        ...
        /// 鼠标在toolbox上移动触发的事件
        /// </summary>
        public event TabMouseMoveHandler ItemMouseMove;
        public event TabMouseMoveHandler ItemMouseEnter;
        ...
        private void Initialize() {
            ...
            _textBox.ImeMode = System.Windows.Forms.ImeMode.Off;
        ...
        internal void SetArrowScrollImage(Image arrowimg) {
            this._dnScroll.ArrowBtnImg = arrowimg;
            this._upScroll.ArrowBtnImg = arrowimg;
        }
        ...
        ///-------------------------------------------------------------------------------------------------
        /// <summary>
        ///     Gets or sets a value indicating whether the only one item per row is shown.
        /// </summary>
        ///SGD:只显示1列
        /// <value> true if show only one item per row, false if not. </value>
        ///-------------------------------------------------------------------------------------------------
        ...
        [Category("ToolBox")]
        public int TabCount {
            get { if (_toolBoxTabs == null)return 0; return _toolBoxTabs.Count; }

        }
        /// <summary>
        /// 对imageIndex大于index的所有item降序号
        /// smallorlargeall:
        /// 0:small
        /// 1:large
        /// 2:all
        /// 在删除 imagelist内的图片后使用
        /// </summary>
        /// <param name="index"></param>
        /// <param name="smallorlargeall"></param>
        public void MoveDownImgIndex(int imgindex, int smallorlargeall) {
            if (_toolBoxTabs == null) return;

            if (smallorlargeall == 0) {
                int tc = TabCount;
                for (int i = 0; i < tc; i++) {
                    if (_toolBoxTabs[i].SmallImageIndex > imgindex) {
                        _toolBoxTabs[i].SmallImageIndex--;
                    }
                    int ic = _toolBoxTabs[i].ItemCount;
                    for (int j = 0; j < ic; j++) {
                        if (_toolBoxTabs[i][j].SmallImageIndex > imgindex) {
                            _toolBoxTabs[i][j].SmallImageIndex--;
                        }
                    }
                }
            }
            else if (smallorlargeall == 1) {
                int tc = TabCount;
                for (int i = 0; i < tc; i++) {
                    if (_toolBoxTabs[i].LargeImageIndex > imgindex) {
                        _toolBoxTabs[i].LargeImageIndex--;
                    }
                    int ic = _toolBoxTabs[i].ItemCount;
                    for (int j = 0; j < ic; j++) {
                        if (_toolBoxTabs[i][j].LargeImageIndex > imgindex) {
                            _toolBoxTabs[i][j].LargeImageIndex--;
                        }
                    }
                }
            }
            else if (smallorlargeall == 2) {
                int tc = TabCount;
                for (int i = 0; i < tc; i++) {
                    if (_toolBoxTabs[i].SmallImageIndex > imgindex) {
                        _toolBoxTabs[i].SmallImageIndex--;
        ...
        */
        ...
        internal void UpdateToolTip(ToolObject obj, MouseEventArgs e) {
            if (null == obj.ToolTip || 0 >= obj.ToolTip.Length && obj.ForceCaptionToolTip || !obj.FullyVisible) {
                UpdateToolTip(obj, obj.Caption, e);
            }
            else {
                UpdateToolTip(obj, obj.ToolTip, e);
            }

        }
        private ToolObject _oldHoverObj = null;
        internal void UpdateToolTip(ToolObject obj, string toolTip, MouseEventArgs e) {
            //if(!toolTip.Equals(_toolTip.GetToolTip(this)))
            //{
            //    _toolTip.SetToolTip(this,toolTip);
            //}
            //if (_oldHoverObj != obj)
            //    System.Diagnostics.Debug.WriteLine(obj == null ? "null" : obj.GetType().ToString());
            if (ItemMouseMove != null) {

                ItemMouseMove(obj, toolTip, e);
            }
            if (_oldHoverObj != obj && ItemMouseEnter != null) {
                //可定义为进入时的事件  
                _oldHoverObj = obj;
                ItemMouseEnter(obj, toolTip, e);
            }
        }
        ...
        private void PaintBackground(Graphics g, Rectangle clipRect) {
            ...
            //绘制背景图
            if (this.Image != null) {
                g.DrawImage(this.Image, clipRect, new Rectangle(0, 0, this.Image.Width, this.Image.Height), GraphicsUnit.Pixel);
            }
        }
        ...
        protected override void Dispose(bool disposing) {
            ...
            if (disposing) {
                ...
                if (this._toolBoxTabs != null) {
                    this._toolBoxTabs.Clear();
                    this._toolBoxTabs = null;
                }
            }
            this._largeImageList.Dispose();
            this._smallImageList.Dispose();
        ...
        protected override void OnMouseMove(MouseEventArgs e) {
            ...
            if (!LayoutTimerActive && !_textBox.Visible) {
                //UpdateToolTip("");
                bool oldinvateUp = _upScroll.MouseHover;
                bool oldinvateDn = _dnScroll.MouseHover;
                _upScroll.MouseHover = false;
                _dnScroll.MouseHover = false;
                if (_upScroll.HitTest(e.X, e.Y)) {
                    _upScroll.MouseHover = true;
                    if (null != _selectedTab) {
                        _selectedTab.CancelHotItemHover();
                        _selectedTab.CancelHover();
                    }
                    UpdateToolTip(_upScroll, e);
                }
                else if (_dnScroll.HitTest(e.X, e.Y)) {
                    _dnScroll.MouseHover = true;
                    if (null != _selectedTab) {
                        _selectedTab.CancelHotItemHover();
                        _selectedTab.CancelHover();
                    }
                    UpdateToolTip(_dnScroll, e);
                }
                else {
                    //UpdateToolTip("");
                ...
                if (oldinvateUp != _upScroll.MouseHover) {
                    this.Invalidate(_upScroll.Rectangle);
                }
                if (oldinvateDn != _dnScroll.MouseHover) {
                    this.Invalidate(_dnScroll.Rectangle);
                }
            }
        }
