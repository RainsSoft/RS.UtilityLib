    ...
    public class ToolBoxTab : ToolBoxItem {
        ...
        */
        ...
        public ToolBoxItem SelectedItem {
            get { if (_selItemIndex < 0)return null; return this[_selItemIndex]; }
            set {
                if (null != _toolItems) {
        ...
        public ToolBoxTab(string caption, int smallImageIndex)
            : base(caption, smallImageIndex) {
            Initialize();
        }
            ...
            if (_itemArea.Contains(e.X, e.Y)) {
                ...
                if (bFound) {
                    ...
                    if (_hotItemIndex != index) {
                        ...
                        item.Invalidate();
                        _parent.UpdateToolTip(item, e);
                    }
                }
                else {
                    CancelHotItemHover();
                    _parent.UpdateToolTip(null, "", e);//没有找到
                }
        ...
        /// <summary> tab标题图背景The table title image </summary>
        public Image TableTitleBackImg = null;
        private void DoPainting(PaintEventArgs e) {
            ...
            if (_rectangle.IntersectsWith(r)) {
                ...
                //#endif
                if (this.TableTitleBackImg != null) {
                    //Console.WriteLine(r.ToString());
                    //绘制标题背景
                    g.DrawImage(this.TableTitleBackImg, r, new Rectangle(0, 0, this.TableTitleBackImg.Width, this.TableTitleBackImg.Height), GraphicsUnit.Pixel);
                }
                if (_mouseDown) {
                    r.Offset(1, 1);
                }
        ...
        private void DrawImage(Graphics g, ImageList imgList, int index, Rectangle itemRec, Rectangle rImage, bool bEnabled, bool bPartial) {
            if (0 < rImage.Height) {
                if (bPartial) {
                    if (bEnabled) {
                        g.DrawImage(
                    ...
                    if (bEnabled) {
                        //imgList.Draw(g,rImage.X,rImage.Y,rImage.Width,rImage.Height,index);
                        g.DrawImage(
                          imgList.Images[index],
                          itemRec, 0, 0,
                          rImage.Width,
        ...
        private void PaintItems(Graphics g, Rectangle clipRect, StringFormat strFormat) {
            ...
            if (!(0 >= _itemArea.Height) && null != _toolItems) {
                ...
                for (iLoop = _visibleTopIndex; iLoop <= _visibleBottomIndex; iLoop++) {
                    ...
                    rect = DrawItemBack(g, item, rect, bgBrushes);

                    // Draw border
                    rect = DrawItemBorder(g, item, rect);
                    ...
                    // Draw image
                    DrawItemImage(g, ref offset, ref drewImage, item, ref rect, ref rImage, ref rImageEx, imgList, imageIndex);
                    ...
                    //DrawItemText(g, strFormat, ref drawText, drewImage, item, ref rect, ref rImage, txBrushes, ref txBrush);
        ...
        private void DrawItemText(Graphics g, StringFormat strFormat, ref bool drawText, bool drewImage, ToolBoxItem item, ref Rectangle rect, ref Rectangle rImage, Brush[] txBrushes, ref Brush txBrush) {
            drawText = true;
        ...
        private void DrawItemImage(Graphics g, ref int offset, ref bool drewImage, ToolBoxItem item, ref Rectangle rect, ref Rectangle rImage, ref Rectangle rImageEx, ImageList imgList, int imageIndex) {
            if (null != imgList && -1 != imageIndex && imageIndex < imgList.Images.Count) {
                rect.X += 2;
                rect.Width -= 2;

                if (_viewMode == ViewMode.LargeIcons && IsShowItemText) {
                    offset = 2;
                }
                else {
                ...
                if (rImageEx.Bottom > _itemArea.Bottom) {
                    ...
                    DrawImage(g, imgList, imageIndex, rImageEx, rImageEx, Enabled && item.Enabled && _parent.Enabled, true);
                }
                else {
                    if (IsShowHoverItemLarge && View != ViewMode.List && item.MouseHover && !item.MouseDown) {
                        //显示大图
                        int lw = 8; //item.Parent.LargeItemSize.Width - item.Parent.SmallItemSize.Width;
                        int lh = 8;// item.Parent.LargeItemSize.Height - item.Parent.SmallItemSize.Height;
                        Rectangle imglargerr = new Rectangle(rImageEx.X, rImageEx.Y, rImageEx.Width + lw, rImageEx.Height + lh);
                        //Rectangle imglargerrex = new Rectangle(rImageEx.X, rImageEx.Y,item.Parent.LargeItemSize.Width,item.Parent.LargeItemSize.Height);
                        DrawImage(g, imgList, imageIndex, imglargerr, rImageEx, Enabled && item.Enabled && _parent.Enabled, false);
                        // DrawImage(g, item.Parent.LargeImageList, item.LargeImageIndex, imglargerr,imglargerrex, Enabled && item.Enabled && _parent.Enabled, false);
                    }
                    else {
                        DrawImage(g, imgList, imageIndex, rImageEx, rImageEx, Enabled && item.Enabled && _parent.Enabled, false);
                    }
                }
        ...
        private Rectangle DrawItemBorder(Graphics g, ToolBoxItem item, Rectangle rect) {
        ...
        private Rectangle DrawItemBack(Graphics g, ToolBoxItem item, Rectangle rect, Brush[] bgBrushes) {
        ...
        /// <summary> 是否显示文本？true if is show text </summary>
        public bool IsShowItemText = false;

        /// <summary> 是否突出显示图标？true if is show hover item large </summary>
        public bool IsShowHoverItemLarge = true;
        ...
        /// 刷新当前选择界面
        /// </summary>
        public void ReflashSelectItem() {
            if (this._toolItems == null) return;
            for (int i = 0; i < this._toolItems.Count; i++) {
                if (i != _selItemIndex) {
                    this._toolItems[i].Selected = false;
                }
            }
            if (this._toolItems.Count == 0) {
