...
namespace RS.UtilityLib.WinFormCommon.UI.MyScrollBar
{
    public class DockPanelStripRenderer : ToolStripRenderer
    {
        ...
        public DockPanelStripRenderer(Boolean drawBottomBorder, Boolean useTheme) {
            ...
            bool renderMode = false;//useTheme;
            if (renderMode == false) //== UiRenderMode.System)
                renderer = new ToolStripSystemRenderer();
            else
                renderer = new ToolStripProfessionalRenderer(this.colorTable);
        }
        ...
        protected override void InitializeItem(ToolStripItem item) {
            ...
            if (item.Image == null && item.IsOnDropDown) {
                ...
                using (Graphics g = Graphics.FromImage(image)) {
                    g.DrawRectangle(Pens.DarkGray, new Rectangle(0, 0, 32, 32));
                }
                item.Image = image;
            }
        ...
        protected override void OnRenderItemCheck(ToolStripItemImageRenderEventArgs e) {
            if (renderer is ToolStripProfessionalRenderer) {
                ...
                Bitmap image = new Bitmap(32, 32);//PluginBase.MainForm.FindImageAndSetAdjust("485");
                using (Graphics g = Graphics.FromImage(image)) {
                    Rectangle closerRect = new Rectangle(0, 0, 32, 32);
        ...
        protected override void OnRenderItemImage(ToolStripItemImageRenderEventArgs e) {
            if (renderer is ToolStripProfessionalRenderer) {
                ...
                if (e.Item is ToolStripMenuItem && e.Item != null && ((ToolStripMenuItem)e.Item).Checked)
                    return;
            }
