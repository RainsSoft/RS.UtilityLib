...
namespace RS.UtilityLib.WinFormCommon.UI
{
    ...
    public partial class MediaSlider : UserControl
    {
        ...
        public event Action<int> OnStepValueEvent;
        public event Action<int> OnStepValueEndEvent;
        #endregion
        ...
        public MediaSlider() {
            ...
            //IRobotQ.TimerCallback tc = new IRobotQ.TimerCallback(_DoTickStepValue_);
            //m_TimerCall=new IRobotQ.IRQ_ClientTimerCall(null,0.2f, IRobotQ.TimerCallType.Every,tc);
            //IRobotQ.TimerCallManager.Singleton.RegisteTimerCall("MediaSlider_StepValue_TimeTick", m_TimerCall);
        }
        #region 步进状态
       
        void _DoTickStepValue_(object o) {
            Application_Idle(null, null);
        }
        /// <summary>
        /// 步进状态
        /// </summary>
        enum ValueSteppingState
        {
            None,
            Add,
            Down
        ...
        /// 绑定控件
        /// </summary>
        bool _StepValueBingCtl = true;
        long _StepValueLastTick = 0;
        int _StepValue = 0;
        ValueSteppingState _ValueSteppingState = ValueSteppingState.None;
        const int _STEPVALUE_DELATTIME_ = 200;//200毫米更新一次，每秒增加5个值
        int TickCount=0;
        DateTimeOffset TickTime=DateTimeOffset.MinValue;
        void Application_Idle(object sender, EventArgs e) {
            if (TickTime == DateTimeOffset.MinValue) {
                TickTime = DateTimeOffset.Now;
            }
            if ((TickTime - DateTimeOffset.Now).TotalMilliseconds > 20) {
                TickCount++;
                TickTime = DateTimeOffset.Now;
            }
            switch (_ValueSteppingState) {
                case ValueSteppingState.Add:
                    //if (Environment.TickCount - _StepValueLastTick > _STEPVALUE_DELATTIME_) {
                    //更新值                
                    //_StepValueLastTick = Environment.TickCount;
                    OnStepValueAdding();
                    //}
                    break;
                case ValueSteppingState.Down:
                    if (TickCount - _StepValueLastTick > _STEPVALUE_DELATTIME_) {
                        //更新值                
                        _StepValueLastTick = TickCount;
                        OnStepValueDowning();
                    }
                    break;
                default:
        ...
        public void SetStepValueAdding_Begin() {
            _StepValue = 0;
            _ValueSteppingState = ValueSteppingState.Add;
        }
        public void SetStepValueAdding_End() {
            _ValueSteppingState = ValueSteppingState.None;
            if (_StepValueBingCtl && Scrolled_End != null) {
                Scrolled_End(this, new EventArgs());
            }
            if (OnStepValueEndEvent != null) {
                OnStepValueEndEvent(_StepValue == 0 ? 1 : _StepValue);
            }
        }
        /// <summary>
        /// 跳到下一帧
        /// </summary>
        public void SetStepValueAdd_Next() {
            if (OnStepValueEndEvent != null) {
                OnStepValueEndEvent(1);
            }
        }
        public void SetStepValueAdd_Back() {
            if (OnStepValueEndEvent != null) {
                OnStepValueEndEvent(-1);
            }
        }
        public void SetStepValueDowning_Begin() {
            _StepValue = 0;
            _ValueSteppingState = ValueSteppingState.Down;
        }
        public void SetStepValueDowing_End() {
            _ValueSteppingState = ValueSteppingState.None;
            if (_StepValueBingCtl) {
                if (Scrolled_End != null) {
                    Scrolled_End(this, new EventArgs());
                }
            }
            else if (OnStepValueEndEvent != null) {
                OnStepValueEndEvent(_StepValue);
            }
        }
        void OnStepValueAdding() {
            if (_StepValueBingCtl) {
                int i = this.Value + 1;
                if (i >= this.Maximum) {
                    _ValueSteppingState = ValueSteppingState.None;
                ...
                _StepValue++;
                if (OnStepValueEvent != null) {
                    OnStepValueEvent(_StepValue);
                }
            }
        }
        void OnStepValueDowning() {
            if (_StepValueBingCtl) {
                int i = this.Value - 1;
                if (i < this.Minimum) {
                    _ValueSteppingState = ValueSteppingState.None;
