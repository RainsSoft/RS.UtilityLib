...
namespace AnimatedGif {
    public class AnimatedGifCreator : IDisposable {
        ...
        public AnimatedGifCreator(Stream stream)
            : this(stream, 33, 0) {

        }
        public AnimatedGifCreator(string filePath)
            : this(filePath, 33, 0) {

        }
        public AnimatedGifCreator(Stream stream, int delay, int repeat) {
            Delay = delay;
            Repeat = repeat;

            _stream = stream;
        }
        public AnimatedGifCreator(string filePath, int delay, int repeat) {
            FilePath = filePath;
            Delay = delay;
            Repeat = repeat;
        ...
        public void AddFrame(Image image) {
            AddFrame(image, -1, GifQuality.Default);
        }
        /// <summary>
        ...
        public void AddFrame(Image image, int delay, GifQuality quality) {
        ...
        public void AddFrame(string path) {
            AddFrame(path, -1, GifQuality.Default);
        }
        /// <summary>
        ///     Add a new frame to the GIF
        /// </summary>
        ...
        /// <param name="delay">The delay in milliseconds this GIF will be delayed (-1: Indicating class property delay)</param>
        /// <param name="quality">The GIFs quality</param>
        public void AddFrame(string path, int delay, GifQuality quality) {
