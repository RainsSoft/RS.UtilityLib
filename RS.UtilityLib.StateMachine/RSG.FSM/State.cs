...
namespace RS.UtilityLib.StateMachine.RSG
{
    ...
    public interface IState
    {
        ...
        /// <summary>
        /// [root state] Update this state and its children with a specified delta time.
        /// </summary>
    ...
    public abstract class AbstractState : IState
    {
        ...
        void addStateInstance2Root(IState state) {
            ...
            //todo:check state duplicate name
            foreach (var item in rs.m_allChildStateInRoot) {
                if (string.Compare(item.StateName, state.StateName, true) == 0) {
                    throw new Exception("rootstate already exists state name:" + item.StateName);
                }
            }
        ...
        public void ChangeState(string stateName) {
            ...
            if (activeChildren.Count > 0) {
                (activeChildren.Pop() as AbstractState).Exit();
            }

            // Activate the new state
            activeChildren.Push(newState);
            (newState as AbstractState).Enter();
        }
        ...
        /// </summary>
        internal void AddChild(IState newState, string stateName) {
            try {
                (newState as AbstractState).StateName = stateName;
        ...
        ///// </summary>
        //internal void AddChild(IState newState) {
    ...
    {
        internal State(string rootStateName) : base() {
            this.StateName = rootStateName;
#if DEBUG
            //Console.WriteLine("state:"+this.GetHashCode());
#endif
        }

        public static string GetStateDefaultName(IState newState) {
            var name = newState.GetType().Name;
            return name;
        }
        public static string GetStateDefaultName<T>() where T : AbstractState {
