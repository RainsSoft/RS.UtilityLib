    ...
    public sealed class FsmStateBehaviour<T>
    {
        ...
        internal void TriggerUpdate(FsmStateData<T> data) {
            ...
            for (var i = 0; i < conditions.Count; i++) {
                if (conditions[i].Predicate(data)) {
                    conditions[i].Action(data);
                }
            }
            //events 
            lock (events_executeInUpdate) {
                string[] eKeys = new string[events_executeInUpdate.Count];
                events_executeInUpdate.Keys.CopyTo(eKeys, 0);
                //var eKeys = events_executeInUpdate.Keys;
                foreach (var key in eKeys) {
                    TriggerEventArg eValue;
                    if (events_executeInUpdate.TryGetValue(key, out eValue)) {
                        events_executeInUpdate.Remove(key);
                        eValue.TEvent(this.State, eValue.TArgs);
                    }
                }
            }
        }
        /// <summary>
        /// enter state after and the update cout
        /// 进入state后的更新次数
        /// </summary>
        ...
        /// </summary>
        private readonly IDictionary<string, Action<T, EventArgs>> events = new Dictionary<string, Action<T, EventArgs>>();
        private readonly Dictionary<string, TriggerEventArg> events_executeInUpdate = new Dictionary<string, TriggerEventArg>();
        /// <summary>
        ...
        /// </summary>
        public FsmStateBehaviour<T> SetEvent(string identifier, Action<T, EventArgs> eventTriggeredAction) {
        ...
        /// to trigger it.
        /// </summary>
        public FsmStateBehaviour<T> SetEvent<TEvent>(string identifier, Action<T, TEvent> eventTriggeredAction)
            where TEvent : EventArgs {
            events.Add(identifier, (sesion, args) => eventTriggeredAction(sesion, CheckEventArgs<TEvent>(identifier, args)));
            return this;
        }
        ...
        /// <param name="executeEventInUpdate">如果为true,则是线程安全的, if true zhen thread safely</param>
        public void TriggerEvent(string name, bool executeEventInUpdate = false) {
            TriggerEvent(name, EventArgs.Empty, executeEventInUpdate);
        }
        ...
        /// <param name="executeEventInUpdate">如果为true,则是线程安全的, if true zhen thread safely</param>
        public void TriggerEvent(string name, EventArgs eventArgs, bool executeEventInUpdate = false) {
            //
            Action<T, EventArgs> myEvent;
            if (events.TryGetValue(name, out myEvent)) {
                if (executeEventInUpdate) {
                    //thread safely
                    TriggerEventArg tea = new TriggerEventArg() {
                        TArgs = eventArgs,
                        TEvent = myEvent
                    };
                    lock (events_executeInUpdate) {
                        //events_executeInUpdate.AddOrUpdate(name, tea, (key, oldvalue) => tea);
                        if (events_executeInUpdate.ContainsKey(name)) {
                            events_executeInUpdate[name] = tea;
                        }
                        else {
                            events_executeInUpdate.Add(name, tea);
                        }
                    }
                }
                else {
                    //thread unsafely
                    myEvent(this.State, eventArgs);
                }
            }

        }
        private struct TriggerEventArg
        {
            public EventArgs TArgs;
            public Action<T, EventArgs> TEvent;
        }
        #endregion
        ...
        {
            public Func<FsmStateData<T>, bool> Predicate;
            public Action<FsmStateData<T>> Action;
        }
        ...
        /// predicate is true.
        /// </summary>
        public FsmStateBehaviour<T> SetCondition(Func<FsmStateData<T>, bool> predicate, Action<FsmStateData<T>> action) {
